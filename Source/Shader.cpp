...
VkShaderModule createShaderModule(VkDevice device, std::vector<uint32_t> code)
{
...
void createShaderModules(VkDevice device, std::vector<ShaderStage> &stages)
{
    for(auto &stage : stages)
        stage.module = createShaderModule(device, stage.spirv);
}

Shader ShaderManager::createShaderStages(std::string vertexFile, std::string fragmentFile)
{
    std::vector<ShaderStage> stages = {
            ShaderStage {
            .stage = VK_SHADER_STAGE_VERTEX_BIT,
            .spirv = loadSPIRV(vertexFile)
        },
            ShaderStage {
            .stage = VK_SHADER_STAGE_FRAGMENT_BIT,
            .spirv = loadSPIRV(fragmentFile)
        }
    };
